...
void SpeechManager::speechContest()
{
	...
	for (vector<int>::iterator it = v_Src.begin(); it != v_Src.end(); it++)
	{
		...
		//打印平均分
		//cout << "编号： " << *it << " 姓名： " << this->m_Speaker[*it].m_Name << " 获得平均分： " << avg << endl;

		//将平均分放入 map容器
		this->m_Speaker[*it].m_Score[this->m_Index - 1] = avg;

		//将分数和选手编号 放入临时容器
		groupScore.insert(make_pair(avg, *it)); //key是分数, value是选手编号
		//每6人取出前三名
		if (num % 6 == 0)
		{
			cout << "第" << num / 6 << "小组比赛成绩： " << endl;
...
void SpeechManager::loadRecord()
{
	...
	//{
	//	cout << it->first << "届冠军编号： " << it->second[0] << " 分数： " << it->second[1] << endl;
	//}
}
